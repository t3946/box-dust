...
@section('content')
    <div class="page-content edit-add container-fluid csItemEditPage">
        <div class="row">
            <div class="col-md-12">
                <div class="panel panel-bordered">
                    <div class="panel-body">
                        <h1>{{$csItem->type}} {{ $csItem->name }} ({{$csItem->type}} {{ $csItem->name_ru }})</h1>
                        <img class="csItemImage" src="{{ Storage::disk('s3')->url($csItem->image) }}" />
                        <form class="saveImageForm">
                        ...
                        <div class="panel-body">
                            ...
                            @if (isset($box_items))
                                <div class="form-group col-md-12 ">
                                    <h2 class="page-header-2">Что в коробке
                                        ({{array_reduce($box_items, static fn ($carry, $group) => $carry += count($group))}}
                                        ):</h2>
                                    <div class="rare-group">
                                        @foreach($box_items as $rare_slug => $rare_group)
                                            <h3 class="page-header-3 rare-group__header rare-group-header_{{$rare_group[0]->rare_slug}}">{{$rare_group[0]->rare_name}}
                                                ({{count($rare_group)}}):</h3>
                                            <ul class="list-unstyled rare-group__list">
                                                @foreach($rare_group as $box_item)
                                                    <li class="rare-group__list-item">{{$box_item->name}}
                                                        <strong>({{number_format($box_item->price, 0, ',', '.')}}
                                                            руб.)</strong>
                                                    </li>
                                                @endforeach
                                            </ul>
