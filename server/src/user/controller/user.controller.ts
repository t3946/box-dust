...
export class UserController {
  ...
  public async register(
    ...
    const { email, name, password, applyAgreement } = createUserDto;

    if (!applyAgreement) {
      res.json({
        errors: {
          applyAgreement: 'Вы должны принять пользовательское соглашение',
        },
      });
  ...
  private async sendRegisterConfirmEmail(email, code) {
    ...
    await this.mailService.registrationCheckEmail(email, text, html);
  }

  @Get('register/send-confirm-email')
  public async sendConfirmEmail(@Res() res, @Query() query) {
    const { email } = query;
    const user = await prisma.box_users.findFirst({
